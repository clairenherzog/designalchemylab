...
export default function DesignAlchemyLab() {
  ...
  const ConicalFlask = ({ flask, index, isActive, pouring }) => (
    <div className="relative w-full h-full">
      <img 
        src="https://www.dropbox.com/scl/fi/va10tuo65cjz5c8bqo0b6/7132498a-34fb-4353-8126-118f04db6a57.jpg?rlkey=17x16l8wjn7irkdeqfg4o14vs&raw=1"
        alt="Conical Flask"
        className="absolute inset-0 w-full h-full object-contain"
        crossOrigin="anonymous"
      />
      
      {isActive && !flask.filled && !pouring && (
        <div className="absolute inset-0 border-4 border-amber-400 rounded-lg animate-pulse"></div>
      )}
      ...
      {flask.filled && (
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 w-3/4 h-2/3 animate-in fade-in slide-in-from-bottom duration-1000"
          style={{
            ...
            clipPath: 'polygon(30% 20%, 70% 20%, 100% 100%, 0% 100%)',
            opacity: 0.7
          }}>
    ...
    <div className="relative w-full h-full">
      ...
      {flask.filled && (
        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 w-4/5 h-3/4 animate-in fade-in slide-in-from-bottom duration-1000"
          style={{
            background: flask.color,
            clipPath: 'polygon(8% 0%, 92% 0%, 95% 100%, 5% 100%)',
            opacity: 0.7
          }}>
          <div className="absolute inset-0 bg-gradient-to-t from-transparent via-white/30 to-transparent animate-pulse"></div>
        </div>
        ...
        <svg viewBox="0 0 200 300" className="absolute inset-0 w-full h-full animate-in fade-in slide-in-from-bottom duration-1000" xmlns="http://www.w3.org/2000/svg">
          <defs>
            <linearGradient id={`liquidGrad3${index}`} x1="0%" y1="0%" x2="0%" y2="100%">
              <stop offset="0%" style={{stopColor: flask.color.includes('gradient') ? '#32CD32' : flask.color, stopOpacity: 0.7}} />
              <stop offset="100%" style={{stopColor: flask.color.includes('gradient') ? '#00CED1' : flask.color, stopOpacity: 0.9}} />
            </linearGradient>
          </defs>
              ...
              return (
                <div key={index} className="relative">
                  <div className={`relative transition-all duration-500 ${
                    isActive && !pouring ? 'scale-110' : 'scale-100'
                  } ${flaskType === 'conical' ? 'w-48 h-72' : flaskType === 'beaker' ? 'w-40 h-64' : 'w-48 h-72'}`}>
